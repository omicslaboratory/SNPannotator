...
if(!is.null(output_list$config$paths$output.string.network.image.file) & 
  ...
  htmltools::img(src = knitr::image_uri(output_list$config$paths$output.string.network.image.file), 
...
if(output_list$config$STRING_DB$enrichment_image == TRUE)
  cat('<h4>Functional enrichment visualizations</h4>\n')
