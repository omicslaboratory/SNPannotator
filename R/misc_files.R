...
writeXLSXfile <- function(output,project_info_tbl,fileName)
{
  ...
  if(!file.exists(fileName))
  {
    ...
    # summary table
    snptbl <- NULL
    snptbl <- output[, .(linked_variant_count = sum(gSNP != Linked_SNP)), by = gSNP]
    names(snptbl) <- c('Variant','Proxies')

    writeData(wb, sheet = "Project info", x = "Proxy count per lead variant", startRow = (3 + ncol(project_info_tbl) + 2))
    addStyle(wb = wb,sheet =  "Project info",style =createStyle(fontSize = 16, textDecoration = "bold") ,
             rows = (3 + ncol(project_info_tbl) + 2),
             cols = 1)
    ...
    writeData(wb, sheet = "Project info",
              x = snptbl,
              startRow = (3 + ncol(project_info_tbl) + 2 + 2))

    addStyle(wb = wb,sheet =  "Project info",style =headerStyle ,
             rows = (3 + ncol(project_info_tbl) + 2 + 2),
             cols = c(1:2))

    addStyle(wb = wb,sheet =  "Project info",style =createStyle(halign = 'center',valign = 'center') ,
             rows = c((3 + ncol(project_info_tbl) + 2 + 2 + 1):(3 + ncol(project_info_tbl) + 2 + 2 + 1 + nrow(snptbl))),
             cols = c(1:2),
             gridExpand = TRUE)
    ...
    writeData(wb, sheet = "Project info",
              ...
              startRow = (3 + ncol(project_info_tbl) + 2 + 2 + nrow(snptbl) + 2))

    addStyle(wb = wb,sheet =  "Project info", style =createStyle(fontSize = 12, textDecoration = c("italic","bold")) ,
             cols = 1,
             rows = (3 + ncol(project_info_tbl) + 2 + 2 + nrow(snptbl) + 2))
...
appendXLSXfile <- function(output,thisSheetName,fileName,addFirst = FALSE)
{
  ...
  if(is.character(output))
    output <- data.table::data.table(data = output)
  ...
  tryCatch({
    ...
    if(is.element("p_value",names(output)))
    {
      ...
      output$p_value <- suppressWarnings(as.numeric(output$p_value))
...
appendXLSX_qtl_report <- function(output,sheetName,eqtl.cluster.report,fileName)
{
  ...
  if(is.element('rsid',names(output)))
  {
    a1 <- output[,.N,by=rsid]
  ...
  if(is.element('Gene',names(output)))
  {
    a2 <- output[,.N,by=Gene]
  ...
  if(is.element('Tissue',names(output)))
  {
...
keep_nearest_gene_in_row <- function(gene,geneId) {
  ...
  if (length(dist_values) > 0) {
    ...
    dist_nums <- suppressWarnings(as.numeric(sub(".*dist=(\\d+)\\).*", "\\1", values)))
...
unlist_list_columns <- function(dt) {
  ...
  for (col in cols) {
    dt[, (col) := sapply(get(col), function(x) {
      if (length(x) == 0) NA else unlist(x)[1]
    })]
  }

  # convert p_value to numeric for correct format
  if(is.element("p_value",names(dt))  && !is.numeric(dt$p_value))
  {
    dt$p_value <- suppressWarnings(as.numeric(dt$p_value))
  }
  return(dt)
}
